...
Check that the AKS context is ready:
...
kubectl get applications -n argocd
kubectl describe application uit-course -n argocd
```

Refresh application state:

```powershell
kubectl annotate application uit-course -n argocd argocd.argoproj.io/refresh=hard --overwrite
```
...
The script:
1. Creates or updates `argocd-notifications-secret` in the `argocd` namespace.
...
```text
Username: admin
Password: get the initial admin password from the argocd-initial-admin-secret, then change it after login.
```
